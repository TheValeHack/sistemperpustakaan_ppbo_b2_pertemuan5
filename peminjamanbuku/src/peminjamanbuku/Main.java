...
public class Main {
	...
	public static void menuAwal() {
		...
		if(daftarUser.isEmpty()) {
			...
			for(int i = 0; i < daftarUser.size(); i++) {
				System.out.println((i+1) + ". "+ daftarUser.get(i).getNama() + " [" + daftarUser.get(i).getRole() + "]");
			}
		}
		System.out.println("---------------------------------");
		System.out.println();
	...
	public static void tampilkanDaftarBuku() {
		...
		if(perpustakaan.getDaftarBuku().isEmpty()) {
			System.out.println("-");
		} else {
			for(int i = 0; i < perpustakaan.getDaftarBuku().size(); i++) {
				System.out.println((i+1) + ". "+ perpustakaan.getDaftarBuku().get(i).getJudul());
			}
		}
	}
	public static void aksiMenu(String role, int pilihan) {
		if (role == "pengunjung") {
			Scanner input = new Scanner(System.in);
			if(pilihan == 1) {
				int banyakBuku = perpustakaan.getDaftarBuku().size();
				if(banyakBuku > 0) {
					tampilkanDaftarBuku();
					System.out.println();				
					...
					if((pilihanBuku > banyakBuku) || (pilihanBuku < 1)) {
						...
						Buku bukuTerpilih = perpustakaan.getDaftarBuku().get(pilihanBuku-1);
						if(bukuTerpilih.cekBuku()) {
							...
							for(int i = 0; i < banyakUser; i++) {
								...
								if(userCek.getRole().equals("pustakawan")) {
									nomorUser++;
									System.out.println(nomorUser + ". " + userCek.getNama());
							...
							System.out.println("Buku '" + bukuTerpilih.getJudul() + "' tidak tersedia. Semua buku tersebut sedang dipinjam");
						}
					}
				} else {
				...
				if(banyakBuku > 0) {
					...
					for(int i = 0; i < banyakBuku; i++) {
						Buku bukunya = perpustakaan.getDaftarBuku().get(i);
						for(int j = 0; j < bukunya.getPeminjam().size(); j++) {
							ArrayList<Object> dataPeminjam = bukunya.getPeminjam().get(j);
							if(dataPeminjam.get(1).equals(currentUser)) {
								bukuTidakAda = false;
								nomorBuku++;
								System.out.println(nomorBuku + ". " + bukunya.getJudul());
				...
				if(banyakBuku > 0) {
					...
					if((pilihanBuku > banyakBuku) || (pilihanBuku < 1)) {
						...
						Buku bukuTerpilih = perpustakaan.getDaftarBuku().get(pilihanBuku-1);
						if(bukuTerpilih.cekBuku()) {
							int stokTersisa = bukuTerpilih.getStok() - bukuTerpilih.getPeminjam().size();
							System.out.println("Buku '" + bukuTerpilih.getJudul() + "' tersedia. Masih tersisa " + stokTersisa + " stok yang bisa dipinjam.");
						} else {
							System.out.println("Buku '" + bukuTerpilih.getJudul() + "' tidak tersedia. Semua buku tersebut sedang dipinjam");
						}
					}
					...
					for(int i = 0; i < banyakBuku; i++) {
						...
						for(int j = 0; j < bukunya.getPeminjam().size(); j++) {
							...
							if(dataPeminjam.get(1).equals(currentUser)) {
								...
								System.out.println(nomorBuku + ". Buku '" + bukunya.getJudul() + "' dipinjam dari pustakawan " + dataPeminjam.get(0) + " selama " + dataPeminjam.get(2) + " hari");
								bukuTidakAda = false;
							}
						}
					}
					...
					for(int i = 0; i < banyakBuku; i++) {
						...
						for(int j = 0; j < bukunya.getPeminjam().size(); j++) {
							...
							if(dataPeminjam.get(0).equals(currentUser)) {
								nomorBuku++;
								System.out.println(nomorBuku + "Meminjamkan buku '" + bukunya.getJudul() + "' ke " + dataPeminjam.get(1) + " selama " + dataPeminjam.get(2) + " hari");
								bukuTidakAda = false;
							}
						}
					}
	...
	public static void menuUser() {
		Scanner input = new Scanner(System.in);
		System.out.println();
		System.out.println("Selamat datang " + currentUser.getNama());
		System.out.println("Anda adalah seorang " + currentUser.getRole() + ", anda dapat:");
		System.out.println();
		if(currentUser.getRole().equals("pengunjung")) {
